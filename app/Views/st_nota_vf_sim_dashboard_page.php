                    ...
                    <div class="container-fluid rounded main-bg pt-3">
                        <div class="row mt-1">
                            ...
                            <div class="filter_group_top">
                                <form id="filterFormOmsetTrxTop" method="post" action="<?php echo esc(base_url('/st_nota_vf_sim_dashboard')); ?>" enctype="multipart/form-data">
                                    <?= csrf_field() ?>
...
<script>
     $(document).ready(function() {
        $('#filter_tap').on('change', function () {
            ...
            $.ajax({
                url: "<?= base_url('/st_nota_vf_sim_dashboard/getPicByTap') ?>",
                type: "POST",
                data: {
                    tap: tap,
        ...
        $('#dlImgTop').on('click', function () {
            ...
            const wrapper = document.querySelector('#table_st_nota_vf_simpati_summary .table-responsive');
            ...
            html2canvas(table, {
                ...
                link.download = `table_st_nota_vf_simpati_summary_${Date.now()}.png`;
        ...
        $('#dlImgBot').on('click', function () {
            ...
            const wrapper = document.querySelector('#table_st_nota_vf_simpati_detail .table-responsive');
            ...
            html2canvas(table, {
                ...
                const link = document.createElement('a');
                link.href = canvas.toDataURL('image/png');
                link.download = `table_st_vf_sim_detail_full_${Date.now()}.png`;
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
